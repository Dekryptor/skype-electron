...
function preprocessXmlString(bodyXml) {
    return bodyXml.replace(/&nbsp;/g, ' ')
        ...
        console.log(`match: '${match}' '${p1}' '${offset}' '${str}'`);
...
function translateXml(body, messageId) {
    let parser = new xmldom_1.DOMParser({
        locator: {},
        errorHandler: {
            warning: (w) => console.warn(`Warning: ${w}`),
            error: (w) => console.warn(`Error: ${w} messageId: ${messageId} body: ${body}`),
            fatalError: (w) => console.warn(`Fatal error: ${w}`)
        }
    });
