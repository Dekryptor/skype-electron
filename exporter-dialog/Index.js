class ExporterUi {
    ...
    init() {
        this.exporter.fetchDefaultDbAccounts().then(accounts => {
            this.foundAccounts = this.filterUserAccounts(accounts, this.exporter.username);
            this.fillInitialScreen(this.foundAccounts);
    ...
    fillInitialScreen(profiles) {
        ...
        document.querySelector('.next').addEventListener('click', event => {
            event.preventDefault();
            let accountsToExport = this.getListOfCheckedAccounts();
    ...
    updateProgressBar(percentage) {
        ...
        this.drawCircle(this.progressBackground, 1);
        if (percentage > 0) {
            this.drawCircle(this.progressColor, percentage / 100);
        }
    }
    ...
    displayFoundProfile(username, filePath) {
    ...
    filterUserAccounts(accounts, username) {
